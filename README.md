<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0B0F19,100:22D3EE&height=200&section=header&text=Mohammed%20Shahzan%20Armar&fontSize=42&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Final-Year%20Data%20Science%20Student%20%7C%20ML%20Engineer&descAlignY=58&descSize=18" width="100%"/>

<p>
Building explainable AI for healthcare and edge devices — multi-task deep learning, Grad-CAM, and on-device computer vision.
</p>

<br/>

<a href="https://m-shahzan.github.io/"><img src="https://img.shields.io/badge/Portfolio-22D3EE?style=for-the-badge&logo=googlechrome&logoColor=white"/></a>
<a href="https://www.linkedin.com/in/mohammed-shahzan-armar-2ab1a82a4"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/></a>
<a href="mailto:shahzanarmar01@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/></a>

</div>

<br/>

## 🎓 About Me

I'm a final-year **B.E. in Computer Science and Engineering (Data Science)** student at Anjuman Institute of Technology and Management (VTU), focused on building explainable and deployable machine learning systems.

```yaml
current_focus:
  - 🔬 RetinaXAI: Multi-task diabetic retinopathy & DME detection (Grad-CAM explainability)
  - 👁️ SightLite: On-device visual perception system for lightweight browser agents
  - 📈 Structured ML curriculum: classical models → gradient boosting → applied deployment
  - 🌱 Currently learning: XGBoost, regression modeling, MLOps fundamentals
```

<br/>

## 🛠️ Tech Stack

<table>
<tr>
<td valign="top" width="50%">

**Languages**
<br/>
<img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white"/>
<img src="https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=postgresql&logoColor=white"/>

**Machine Learning & Deep Learning**
<br/>
<img src="https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white"/>
<img src="https://img.shields.io/badge/TensorFlow-FF6F00?style=flat-square&logo=tensorflow&logoColor=white"/>
<img src="https://img.shields.io/badge/scikit--learn-F7931E?style=flat-square&logo=scikitlearn&logoColor=white"/>
<img src="https://img.shields.io/badge/OpenCV-5C3EE8?style=flat-square&logo=opencv&logoColor=white"/>
<img src="https://img.shields.io/badge/XGBoost-006ACC?style=flat-square"/>

</td>
<td valign="top" width="50%">

**Data Analysis & Visualization**
<br/>
<img src="https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white"/>
<img src="https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white"/>
<img src="https://img.shields.io/badge/Matplotlib-11557C?style=flat-square"/>
<img src="https://img.shields.io/badge/Tableau-E97627?style=flat-square&logo=tableau&logoColor=white"/>
<img src="https://img.shields.io/badge/Power%20BI-F2C811?style=flat-square&logo=powerbi&logoColor=black"/>

**Tools & Deployment**
<br/>
<img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white"/>
<img src="https://img.shields.io/badge/Flask-000000?style=flat-square&logo=flask&logoColor=white"/>
<img src="https://img.shields.io/badge/Streamlit-FF4B4B?style=flat-square&logo=streamlit&logoColor=white"/>
<img src="https://img.shields.io/badge/Jupyter-F37626?style=flat-square&logo=jupyter&logoColor=white"/>
<img src="https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black"/>
<img src="https://img.shields.io/badge/Kaggle-20BEFF?style=flat-square&logo=kaggle&logoColor=white"/>

</td>
</tr>
</table>

<br/>

## 🚩 Featured Projects

<table>
<tr>
<td width="50%" valign="top">

### 🩺 [RetinaXAI](https://github.com/M-shahzan/Retinaxai)
Multi-task diabetic retinopathy & DME detection with Grad-CAM explainability.

**0.92 ROC-AUC (DME)** on IDRiD dataset

`PyTorch` `EfficientNet-B3` `Grad-CAM` `Streamlit`

</td>
<td width="50%" valign="top">

### 👁️ [SightLite](https://github.com/M-shahzan/SightLite)
On-device visual perception system for browser agents.

**SIH 2026 · SIH26171** — backed by ISRO

`ViT` `Edge AI` `Hybrid Architecture`

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🚢 [Titanic Survival Prediction](https://github.com/M-shahzan/titanic-survival-ml)
End-to-end Kaggle ML pipeline — EDA, feature engineering, model comparison.

**CV F1: 77.6%** · Kaggle **0.77751** (top ~23%)

`Scikit-learn` `Random Forest`

</td>
<td width="50%" valign="top">

### 📦 [Smart Procurement Hub](https://github.com/M-shahzan/supplier-recommendation)
AI-assisted supplier recommendation platform for businesses.

Random Forest + KNN + weighted scoring engine

`Flask` `Random Forest` `KNN` `Pandas`

</td>
</tr>
</table>

<br/>

## 📊 GitHub Stats

<div align="center">
<img src="https://github-readme-stats.vercel.app/api?username=M-shahzan&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0B0F19&title_color=22D3EE&icon_color=22D3EE&text_color=F1F5F9" alt="GitHub Stats" height="165"/>
<img src="https://github-readme-streak-stats.herokuapp.com/?user=M-shahzan&theme=tokyonight&hide_border=true&background=0B0F19&ring=22D3EE&fire=22D3EE&currStreakLabel=22D3EE" alt="GitHub Streak" height="165"/>
</div>

<div align="center">
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=M-shahzan&layout=compact&theme=tokyonight&hide_border=true&bg_color=0B0F19&title_color=22D3EE&text_color=F1F5F9" alt="Top Languages"/>
</div>

<br/>

## 🐍 Contribution Activity

<div align="center">
<img src="https://raw.githubusercontent.com/M-shahzan/M-shahzan/output/github-contribution-grid-snake.svg" alt="Contribution Snake" width="100%"/>
</div>

<br/>

<div align="center">

*Open to internships, collaborations, or just a conversation about machine learning and AI.*

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:22D3EE,100:0B0F19&height=100&section=footer" width="100%"/>

</div>
